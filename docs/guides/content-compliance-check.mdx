...
<Note>
  **Multipart constraints.** Flat fields, repeated for arrays (`-F "jurisdictions=us"
  -F "jurisdictions=eu"`, not comma-separated). Up to 2 `image` parts. Total request
  body capped at 4.5 MB. Sync-only — `responseMode`, `webhookUrl`, and `callbackId`
  are ignored.
</Note>

<Warning>
  **Cache bypass with images.** The response cache is keyed on text only, so requests
...
- Whether research-stage caches hit (Perplexity Stage A is cached with a 30-day TTL; identical
